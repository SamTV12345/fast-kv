...
test('Key value set', ()=>{
  const db = new Dirty("./test1.dirty")
  db.set('key:test', 'value:test')
  let res = db.get('key:test')
  expect(res).toBe('value:test')
})
...
test('2 db open', ()=>{
  const db = new Dirty("./test2.dirty")
  db.set('test1', "test2")
  db.close()
  const db2 = new Dirty("./test2.dirty")
  db2.set('test2', 'test2')
  db2.close()
})

test('Key value remove', ()=>{
  const db = new Dirty('./test3.dirty')
  db.set('key:test', 'value:test')
  db.remove('key:test')
  let res = db.get('key:test')
  expect(res).toBeNull()
...
test('Key value findKeys', ()=>{
  const db = new Dirty('./test4.dirty')
  db.set('key:test', 'value:test')
  db.set('key:test2', 'value:test2')
  db.set('key:123', "value:123")
...
test('Key value findKeys rev', ()=>{
  const db = new Dirty('./test5.dirty')
...
test('Key value findKeys none', ()=>{
  const db = new Dirty('./test6.dirty')
  db.set('key:2:test', 'value:test')
  db.set('key:3:test2', 'value:test2')
  db.set('key:4:123', "value:123")
...
test('Key value findKeys 45', ()=>{
  const db = new Dirty('./test7.dirty')
  db.set('key:2:test', 'value:test')
  db.set('key:3:test2', 'value:test2')
  db.set('key:4:123', "value:123")
...
test('findKeys with exclusion works', ()=>{
  const db = new Dirty('./test8.dirty')
...
test('findKeys with no matches works', async ()=>{
  const db = new Dirty('./test9.dirty')
  db.set('key:2:test','test')
...
test('find keys with no wildcards works', async ()=>{
  const db = new Dirty('./test10.dirty')
  db.set('key:2:test','')
  db.set('key:2:testa', '')
...
test('get without table', async ()=>{
  const db = new Dirty('./test11.dirty')
  db.get('234dsfsdfsdf')
})