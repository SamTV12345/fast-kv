...
impl PostgresBackend {
  ...
  async fn query_keys(&self, sql: &str, params: &[&(dyn ToSql + Sync)]) -> Result<Vec<String>> {
...
impl Backend for PostgresBackend {
  ...
  async fn get(&self, key: &str) -> Result<Option<Value>> {
    self
      .query_value("SELECT value FROM store WHERE key = $1", &[&key])
      .await
  }
  ...
  async fn find_keys(&self, key: &str, not_key: Option<&str>) -> Result<Vec<String>> {
    ...
    match not_key {
      ...
      None => {
        self
          .query_keys("SELECT key FROM store WHERE key LIKE $1", &[&pattern])
          .await
      }
    }
  }
