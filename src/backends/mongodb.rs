...
impl Backend for MongoBackend {
  async fn init(&mut self) -> Result<()> {
    ...
    let client = Client::with_options(opts).map_err(|e| UeberError::BackendInit(e.to_string()))?;
  ...
  async fn get(&self, key: &str) -> Result<Option<Value>> {
    ...
    match doc {
      Some(mut d) => {
        ...
        let json: Value = serde_json::to_value(v).map_err(UeberError::Serde)?;
        Ok(Some(json))
      }
      None => Ok(None),
    }
  ...
  async fn set(&self, key: &str, value: &Value) -> Result<()> {
    ...
    coll
      .update_one(doc! { "_id": key }, doc! { "$set": { "value": bson } })
      .upsert(true)
      .await
      .map_err(|e| UeberError::Backend(anyhow::anyhow!(e)))?;
    Ok(())
