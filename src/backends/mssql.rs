...
impl Backend for MssqlBackend {
    ...
    async fn get(&self, key: &str) -> Result<Option<Value>> {
        ...
        let stream = c
            .query("SELECT v FROM store WHERE k = @P1 AND DATALENGTH(k) = DATALENGTH(@P1)", &[&key])
            .await
    ...
    async fn remove(&self, key: &str) -> Result<()> {
        ...
        let mut c = pool
            ...
            .await
            .map_err(|e| UeberError::Backend(anyhow::anyhow!(e.to_string())))?;
        c.execute("DELETE FROM store WHERE k = @P1 AND DATALENGTH(k) = DATALENGTH(@P1)", &[&key])
            .await
            .map_err(|e| UeberError::Backend(anyhow::anyhow!(e)))?;
        Ok(())
    }
    ...
    async fn find_keys(&self, key: &str, not_key: Option<&str>) -> Result<Vec<String>> {
        ...
        let stream = if let Some(nk) = not_key {
            ...
            c.query(
                "SELECT k FROM store WHERE k LIKE @P1",
                &[&pattern.as_str()],
            )
            .await
        }
