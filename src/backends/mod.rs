...
pub async fn factory(type_: &str, settings: &Settings) -> Result<Box<dyn Backend>> {
  match type_ {
    ...
    "dirty_git" => Ok(Box::new(dirty_git::DirtyGitBackend::from_settings(
      settings,
    )?)),
    "elasticsearch" => Ok(Box::new(
      elasticsearch::ElasticsearchBackend::from_settings(settings)?,
    )),
    "surrealdb" => Ok(Box::new(surrealdb::SurrealBackend::from_settings(
      settings,
    )?)),
    "cassandra" => Ok(Box::new(cassandra::CassandraBackend::from_settings(
      settings,
    )?)),
    #[cfg(test)]
