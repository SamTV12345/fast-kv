...
const CREATE_TABLE_SQL: &str = "CREATE TABLE IF NOT EXISTS store (key TEXT PRIMARY KEY, value TEXT)";
...
impl SQLite {
    ...
    pub fn new(filename: String) -> napi::Result<Self> {
        ...
        if filename == ":memory:" {
            ...
            memory_conn.execute(CREATE_TABLE_SQL, []).map_err(SqliteErrorWrapper::from)?;
            conn = Some(memory_conn);
        } else if filename.len() != 0 {
            let file_conn = Connection::open(&filename).map_err(SqliteErrorWrapper::from)?;
            file_conn.execute(CREATE_TABLE_SQL, []).map_err(SqliteErrorWrapper::from)?;
            conn = Some(file_conn)
        } else {
            return Err(napi::Error::new(napi::Status::GenericFailure, "filename is empty".to_string()))
        }
    ...
    pub fn find_keys(&self, key: String, not_key: Option<String>) -> napi::Result<Vec<String>> {
        match &self.db {
            ...
            None=>{
                Err(Error::new(Status::GenericFailure, "Db not initialized".to_string()))
            }
        }
    }
    ...
    pub fn get(&self, key: String) -> napi::Result<Option<String>> {
        match &self.db {
            Some(db)=>{
                let mut prep_get = db.prepare("SELECT value FROM store WHERE key = ?").map_err(SqliteErrorWrapper::from)?;
                ...
                match result.next() {
                    ...
                    Err(e)=>{
                        Err(Error::from(SqliteErrorWrapper::from(e)))
                    }
                }
            }
            None=>{
                Err(napi::Error::new(napi::Status::GenericFailure, "Db not initialized".to_string()))
            }
        }
    }
    ...
    pub fn set(&self, key: String, value: String) -> napi::Result<Option<i32>> {
        match &self.db {
            Some(db)=>{
                let mut prep_get = db.prepare("REPLACE INTO store VALUES (?,?)").map_err(SqliteErrorWrapper::from)?;
                let res = prep_get.execute([key,value]).map_err(SqliteErrorWrapper::from)?;
                Ok(Some(res as i32))
            }
            None=>{
                Err(Error::new(Status::GenericFailure, "Db not initialized".to_string()))
    ...
    pub fn remove(&self, key: String) -> napi::Result<()> {
        match &self.db {
            Some(db)=>{
                let mut prep_statement = db.prepare("DELETE FROM store WHERE key = ?")
                    .map_err(SqliteErrorWrapper::from)?;
                prep_statement.execute([key]).map_err(SqliteErrorWrapper::from)?;
                Ok(())
            }
            None=>{
                Err(Error::new(Status::GenericFailure, "Db not initialized".to_string()))
            ...
            Some(ref mut db)=>{
                ...
                for bulk_ob in bulk_object {
                    if bulk_ob.r#type == "set" {
                        let mut prep_statement = tx.prepare("REPLACE INTO store VALUES (?,?)").map_err(SqliteErrorWrapper::from)?;
                        prep_statement.execute(params![bulk_ob.key, bulk_ob.value.unwrap()]).map_err(SqliteErrorWrapper::from)?;
                    } else {
                        let mut prep_statement = tx.prepare("DELETE FROM store WHERE key = ?").map_err(SqliteErrorWrapper::from)?;
                        prep_statement.execute(params![bulk_ob.key]).map_err(SqliteErrorWrapper::from)?;
                    }
                }
