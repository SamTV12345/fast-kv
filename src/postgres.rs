...
impl Postgres {
    ...
    pub fn new(filename: PostgresSettings) -> napi::Result<Self> {
        let mut client = Client::connect(&format!("host={} user={} password={} dbname={} port={}",
                                                  &filename.host, &filename.user, &filename.password, &filename.database, &filename
                .port), NoTls).map_err(PostgresErrorWrapper::from)?;

        client.execute(CREATE_TABLE_SQL, &[]).map_err(PostgresErrorWrapper::from)?;
        Ok(Postgres{
            db: Some(client),
        })
    }
        ...
        match &mut self.db {
            Some(db) => {
                let prep_get = db
                    ...
                    .map_err(PostgresErrorWrapper::from)?;
                let result = db.query(&prep_get, &[&key])
                    .map_err(PostgresErrorWrapper::from)?;
                match result.first() {
                    Some(r) => {
    ...
    pub fn set(&mut self, key: String, value: String) -> napi::Result<Option<i64>> {
        match &mut self.db {
            Some(db) => {
                let prep_get = db
                    .prepare("INSERT INTO store (key, value) VALUES ($1, $2) ON CONFLICT (key) DO UPDATE SET value =
                    $2")
                    .map_err(PostgresErrorWrapper::from)?;
                let res = db.execute(&prep_get, &[&key, &value])
                    .map_err(PostgresErrorWrapper::from)?;
                Ok(Some(res as i64))
            }
            None => Err(Error::new(
        ...
        match &mut self.db {
            Some(db) => {
                let prep_statement = db
                    ...
                    .map_err(PostgresErrorWrapper::from)?;
                db.execute(&prep_statement, &[&key]).map_err(PostgresErrorWrapper::from)?;
                Ok(())
            }
            None => Err(Error::new(
                Status::GenericFailure,
    ...
    pub fn destroy(&mut self) -> napi::Result<()> {
        if let Some(db) = &mut self.db {
            db.execute("DELETE FROM store", &[]).map_err(PostgresErrorWrapper::from)?;
            Ok(())
        } else {
            Err(Error::new(
                Status::GenericFailure,
